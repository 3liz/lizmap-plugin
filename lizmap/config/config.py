...
class LizmapConfig:
    ...
    def set_global_options(self, options: Optional[Mapping[str, Any]] = None):
        ...
        """
        # set defaults
        self._global_options = { k: v['default'] for k, v in self.globalOptionDefinitions.items()
            if v.get('_api', True) }
        ...
        if len(project_wms_extent) > 1:
            bbox = [
                float(project_wms_extent[0]),
                float(project_wms_extent[1]),
                float(project_wms_extent[2]),
                float(project_wms_extent[3]),
            ]
        else:
            bbox = []
