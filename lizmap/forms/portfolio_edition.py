...
class PortfolioEditionDialog(BaseEditionDialog, CLASS):
    ...
    def layouts_collection(self, saved: bool = False) -> list:
        """Return the list of layouts collection."""
        collection = list()
        rows = self.layouts.rowCount()

        collection_definition = self.config.layer_config['layouts']
        for row in range(rows):
            layout_data = dict()
            for i, sub_key in enumerate(collection_definition['items']):
                # Get the item
                item = self.layouts.item(row, i)
                ...
                layout_data[sub_key] = cell

            collection.append(layout_data)

        return collection

    def save_collection(self):
        """Save a collection into JSON"""
        return self.layouts_collection(True)
    ...
    def validate(self) -> Optional[str]:
        ...
        if self.layouts.rowCount() == 0:
            return tr('At least one layout has to be configured')
