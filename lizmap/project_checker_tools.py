...
def project_invalid_pk(project: QgsProject) -> tuple[list[SourceLayer], list[SourceLayer]]:
    """ Check either non-existing PK or PK different from int4. """
    autogenerated_keys = []
    invalid_pk_type = []
    for layer in project.mapLayers().values():
        ...
        # Test int4 primary keys only for PostgreSQL layers
        if layer.dataProvider().name() != 'postgres':
            continue

        result, _field = auto_generated_primary_key_field(layer)
        if result:
            autogenerated_keys.append(SourceLayer(layer.name(), layer.id()))

        if len(layer.primaryKeyAttributes()) != 0 and not valid_int4_primary_key(layer):
            invalid_pk_type.append(SourceLayer(layer.name(), layer.id()))
            continue

    return autogenerated_keys, invalid_pk_type
...
def auto_generated_primary_key_field(layer: QgsVectorLayer) -> tuple[bool, str | None]:
...
def valid_int4_primary_key(layer: QgsVectorLayer) -> bool:
    """ If the layer has a primary key with int4. """
    # QgsVectorLayer.primaryKeyAttributes is returning a list.
...
def _split_layer_uri(provider: str, source: str) -> tuple[str, str | None]:
...
def duplicated_layer_with_filter(project: QgsProject) -> dict[str, dict[str, str]] | None:
...
def table_type(layer: QgsVectorLayer) -> QgsAbstractDatabaseProviderConnection.TableFlag | None:
