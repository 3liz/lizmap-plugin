...
class ServerManager:
    ...
    def check_lwc_version(self, version_check: str) -> bool:
        ...
        for row in range(self.table.rowCount()):
            lwc_version = self.table.item(row, TableCell.LizmapVersion.value).data(
            ...
            if not result:
                QMessageBox.critical(
                    ...
                    tr(
                        ...
                        "global settings, then 'Authentication' tab."
                    ).format(auth_id),
    ...
    def _edit_row(self, row: int, server_url: str, auth_id: str, name: str):
        ...
        cell.setData(
            ...
            tr("<b>URL</b> {}<br><b>Login</b> {}<br><b>Password ID</b> {}").format(
                server_url, login, auth_id
            ),
        )
    ...
    def fetch(self, url: str, auth_id: str, row: int):
        ...
        if auth_id:
            QgsMessageLog.logMessage(
                f"Using the token for <a href='{url}'>{url}</a>", "Lizmap", Qgis.MessageLevel.Info
            )
    ...
    def request_finished(self, row: int):
        ...
        if reply.error() != QNetworkReply.NetworkError.NoError:
            if reply.error() == QNetworkReply.NetworkError.HostNotFoundError:
                self.display_action(
                    row, Qgis.MessageLevel.Warning, tr("Host can not be found. Is-it an intranet server ?")
                )
        ...
        if branch >= (3, 6):
            ...
            if not qgis_server:
                self.display_action(
                    row, Qgis.MessageLevel.Critical, tr('No "qgis_server" in the JSON document')
                )
                return
        ...
        json_file_content = json.dumps(content, sort_keys=False, indent=4)
        json_file_content += "\n"
        ...
        if git_hash:
            lwc_version_txt += (
                f" - commit {git_hash} https://github.com/3liz/lizmap-web-client/commit/{git_hash}"
            )
        lwc_version_txt += "\n"
        ...
        if branch < (3, 5):
            ...
            qgis_cell.setToolTip(
                tr(
                    "Not possible to determine QGIS Server version because you need at least Lizmap Web Client 3.5"
                )
            )
            qgis_cell.setData(Qt.ItemDataRole.UserRole, markdown)
        ...
        if branch >= (3, 5):
            ...
            if "error" in qgis_server_info:
                if qgis_server_info["error"] in ("NO_ACCESS", "WRONG_CREDENTIALS"):
                    ...
                    qgis_cell.setData(Qt.ItemDataRole.UserRole, markdown)
                    self.update_action_version(
                        lizmap_version, None, row, login, error=qgis_server_info["error"]
                    )
                    return
        ...
        for server in servers:
            ...
            if cache_file.exists():
                with open(cache_file, encoding="utf8") as f:
                    ...
                    LOGGER.info(
                        f"Loading server <a href='{url}'>{name}</a> using cache in the drop down list"
                    )
            else:
    ...
    def save_table(self):
        ...
        json_file_content = json.dumps(data, sort_keys=False, indent=4)
        json_file_content += "\n"
    ...
    def _messages_for_version(
        ...
        for i, json_version in enumerate(json_content):
            ...
            if status == ReleaseStatus.Retired:
                ...
                # Upgrade because the branch is not maintained anymore
                messages.append(
                    tr("Version {version} not maintained anymore, only for security bugfix only").format(
                        version=branch
                    )
                )
            ...
            if json_version["latest_release_version"] != full_version or is_pre_package:
                ...
                if not is_numeric(latest_bugfix):
                    ...
                    if compareVersions(lizmap_version, json_version["latest_release_version"]) == 2:
                        ...
                        messages.append(
                            tr("Not latest bugfix release, {version} is available").format(
                                version=json_version["latest_release_version"]
                            )
                        )
                        level = Qgis.MessageLevel.Warning
        ...
        return (
            Qgis.MessageLevel.Critical,
            [f"Version {branch} has not been detected as a known version."],
            False,
        )
    ...
    def context_menu_requested(self, position: QPoint):
        ...
        is_dev = any(item in version() for item in UNSTABLE_VERSION_PREFIX) or ambiguous_to_bool(
            os.getenv("LIZMAP_ADVANCED_USER")
        )
        if is_dev:
        ...
        action_required = action_item.data(Qt.ItemDataRole.ForegroundRole) in (
            Color.Advice.value,
        ...
        if webdav:
            ...
            qgis_dav = ServerWizard.trailing_slash(
                ServerWizard.trailing_slash(webdav.get("url")) + webdav.get("projects_path")
            )
            left_item = self.table.item(item.row(), TableCell.Url.value)
            url = left_item.data(Qt.ItemDataRole.UserRole)
    ...
    def display_all_versions(self, data: str):
        ...
        QMessageBox.information(self.parent, tr("Server versions"), data, QMessageBox.StandardButton.Ok)
