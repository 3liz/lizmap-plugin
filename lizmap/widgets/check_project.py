...
class Severities:
    ...
    def __init__(self):
        ...
        self.normal = Severity(2, tr("Normal"), tr("This would be nice to have look"), "blue", 2)
        self.low = Severity(3, tr("Low"), tr("Nice to do"), "yellow", 2)
        # Some severities can only be done on runtime, QGIS version and/or Lizmap Cloud
        self.unknown = Severity(99, "Unknown", "Severity will be determined on runtime", "green", 1)
...
class Check:
    ...
    def html_tooltip(self, lizmap_cloud: bool = False) -> str:
        ...
        return (
            f"<strong>{self.description_text(lizmap_cloud)}</strong><br><p>{self.help_text(lizmap_cloud)}</p>"
        )
...
class Checks:
    ...
    def __init__(self):
        ...
        self.OgcValid = Check(
            ...
            tr("According to OGC standard, the project is not valid."),
            (
                "<ul>"
        ...
        self.ServerVersion = Check(
            ...
            ("<ul><li>{upgrade}</li><li>{downgrade}</li></ul>").format(
        ...
        self.NotInt4Pk = Check(
            ...
            ("<ul><li>{help}</li><li>{sql}</li></ul>").format(
                help=tr(
        ...
        self.SSLConnection = Check(
            ...
            tr("Connections to a PostgreSQL database hosted on {} must use a SSL secured connection.").format(
                CLOUD_NAME
            ),
            (f"<ul><li>{qgis_auto_fix_button}</li><li>{global_connection}</li></ul>"),
            Levels.Layer,
            Severities().blocking,
        ...
        self.SimplifyGeometry = Check(
            ...
            ("<ul><li>{auto_fix}</li><li>{help}</li></ul>").format(
        ...
        self.RasterWithoutPyramid = Check(
            ...
            ("<ul><li>{help}</li></ul>").format(
                help=tr("In the raster properties, pyramids panel, it's possible to create it."),
            ),
            Levels.Layer,
            Severities().blocking,
        ...
        self.DuplicatedRuleKeyLegend = Check(
            ...
            tr("Due to a previous bug in QGIS, the project has some duplicated rule key in the legend."),
            ("<ol><li>{}</li><li>{}</li><li>{}</li><li>{}</li><li>{}</li><li>{}</li></ol>").format(
                tr(
        ...
        self.DuplicatedLayerFilterLegend = Check(
            ...
            ("<ol><li>{}</li><li>{}</li></ol>").format(
                tr(
                    "Open the last tab in this panel, to have raw HTML logs, "
                    "you have a table showing all duplicated layers"
                ),
                tr("Remove one of the duplicated layer and remove the filter on the other one."),
            ),
            Levels.Layer,
            Severities().important,
        ...
        self.LayerMissingApiKey = Check(
            ...
            (
                "<ul><li>{}</li><li>{}</li><li>{}</li></ul>".format(
        ...
        self.LeadingTrailingSpaceLayerGroupName = Check(
            ...
            (
                "<ul><li>{edit_layer}</li></ul>".format(
                    edit_layer=tr(
                        "Rename your layer/group to remove leading/trailing spaces (left and right)"
                    ),
                )
            ),
            Levels.Layer,
        ...
        self.PreventEcw = Check(
            ...
            (
                "<ul><li>{help}</li><li>{other}</li></ul>".format(
            ...
            ("<ul><li>{help}</li></ul>").format(help=tr("Switch to a COG format")),
        )
        self.RasterAuthenticationDb = Check(
            ...
            (f"<ul><li>{other_auth}</li><li>{safeguard}</li><li>{global_connection}</li></ul>"),
            Levels.Layer,
            Severities().unknown,
            QIcon(":/images/themes/default/mIconPostgis.svg"),
            tr("The layer is using the QGIS authentication database. This is not compatible with {}").format(
                CLOUD_NAME
            ),
            ("<ul><li>{login_pass}</li></ul>").format(
                login_pass=tr(
        ...
        self.AuthenticationDb = Check(
            ...
            ("<ul><li>{service}</li><li>{login_pass}</li></ul>").format(
        ...
        self.PgForceUserPass = Check(
            ...
            tr("The layer is missing some credentials, either user and/or password."),
            (
                "<ul>"
                "<li>{edit_layer}</li>"
                "<li>{help}</li>"
        ...
        self.PreventDrive = Check(
            ...
            (f"<ul><li>{either_move_file}</li><li>{safeguard}</li></ul>"),
            Levels.Layer,
            Severities().unknown,
        ...
        self.PreventParentFolder = Check(
            ...
            tr("The layer is stored in too many parent's folder, compare to the QGS file."),
            ("<ul><li>{help}</li><li>{other}</li><li>{fyi}</li></ul>").format(
                ...
                ).format(count=CLOUD_MAX_PARENT_FOLDER, hosting_name=CLOUD_NAME),
            ),
        )
...
class TableCheck(QTableWidget):
    ...
    def add_error(
        ...
        if isinstance(error.source_type, SourceType.Layer):
            ...
            layer = QgsProject.instance().mapLayer(error.source_type.layer_id)
            item.setToolTip(tr('Field "{}" in layer name "{}"').format(error.source_type.name, layer.name()))
            # Override the text
