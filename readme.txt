...
Lizmap Qgis plugin aims to be used to configure a web application dynamically generated by Lizmap (php/html/css/js) with the help of Qgis Server ( http://www.qgis.org/wiki/QGIS_Server_Tutorial ) With this plugin, you can configure one web map per Qgis project. The Lizmap web application must be installed on the server.

We like to thank Alisé Géomatique ( http://www.alise-geomatique.fr ) a french company, for the help the provided by reporting bugs and asking for new features during the development of Lizmap Qgis plugin and web application. They founded the developpement of adding the public baselayers support (Google or OpenStreetMap).
