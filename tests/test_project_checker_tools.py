...
class TestProjectTable(TestCase):
    def test_project_invalid_pk(self, data: Path):
        """Test check either non-existing PK or PK different from int4 on non sql layers"""
        project_file = str(data.joinpath("shortnames.qgs"))
        project = QgsProject.instance()
        project.read(project_file)

        autogenerated_keys, not_int4 = project_invalid_pk(project)
        self.assertEqual(0, len(autogenerated_keys))
        self.assertEqual(0, len(not_int4))
