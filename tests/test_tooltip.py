...
class TestToolTip(TestCase):
    def check_layer_context(self, field_value, expression, expected):
        ...
        self.assertFalse(
            expression.hasParserError(), error.format(expression.expression(), expression.parserErrorString())
        )

        expression.prepare(context)
        self.assertFalse(
            expression.hasEvalError(), error.format(expression.expression(), expression.evalErrorString())
        )
