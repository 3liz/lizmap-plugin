...
def test_group_popup_by_layer(data: Path, qgis_iface: QgisInterface):
    """Test group popup by layer UI settings."""
    lizmap = _setup_empty_project(data, qgis_iface)

    # Default checkbox states
    assert not lizmap.dlg.checkbox_group_popup_by_layer.isChecked()

    # Enable group popup by layer
    lizmap.dlg.checkbox_group_popup_by_layer.setChecked(True)
    assert lizmap.dlg.checkbox_group_popup_by_layer.isEnabled()

    output = lizmap.project_config_file(LwcVersions.latest(), check_server=False, ignore_error=True)
    # Group popup by layers option should be checked in output config file
    assert output["options"].get("group_popup_by_layer")
    ...
    assert not output["options"].get("group_popup_by_layer")


def test_short_link_permalink(data: Path, qgis_iface: QgisInterface):
    """Test short link permalink UI settings."""
    lizmap = _setup_empty_project(data, qgis_iface)

    # Default checkbox states
    assert not lizmap.dlg.checkbox_short_link_permalink.isChecked()

    # Enable short link permalink
    lizmap.dlg.checkbox_short_link_permalink.setChecked(True)
    assert lizmap.dlg.checkbox_short_link_permalink.isEnabled()

    output = lizmap.project_config_file(LwcVersions.latest(), check_server=False, ignore_error=True)
    # Short link permalink option should be checked in output config file
    assert output["options"].get("short_link_permalink")
    ...
    assert not output["options"].get("short_link_permalink")
